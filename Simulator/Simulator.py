...
def op_add(regA, regB, destReg):  # add operator
    ...
    # bring register[regA] + register[regB] assign to register[destReg]
...
def op_nand(regA, regB, destReg):  # nand operator
    ...
    count += 1
    global REGISTER  # call register variable from global
    # bring register[regA] nand register[regB] assign to register[destReg]
    REGISTER[destReg] = ~(REGISTER[regA] & REGISTER[regB])
    printState()  # call printState func
...
def op_lw(regA, regB, offset):  # load word operator
    ...
    while REGISTER[regA] + offset >= len(memory):
        memory.append(0)
    # load memory[(REGISTER[regA] + offset)] to register[regB]
...
def op_sw(regA, regB, offset):  # store word operator
    ...
    while REGISTER[regA] + offset >= len(memory):
        memory.append(0)
    # load register[regB] to memory[REGISTER[regA] + offset]
...
def machinecodereader(input):
    ...
    if (str_input[7] == '0' and str_input[8] == '0'):
        # bring position of string at 29-32 to str_rd
        str_rd = str_input[29:32]
        # bring position of string at 10-13 to str_rd
        str_rs = str_input[10:13]
        # bring position of string at 13-16 to str_rd
        ...
        # bring position of string at 16-32 to str_rd
